...
```json
{
  ...
  "omitidos": [{"num": "5.2", "motivo": "1,1% da prova e confiança 0,28 — fora do recorte"}]
}
```
...
| `item`, `modulo`, `importancia` | do `plano.json`, sem alterar |
...
diferente do plano, importância alterada e ordem que viola pré-requisito.
...
`tipo_cobranca`, `marcas`, `requer`, `destrava`, `modulo` e `obs`, mais `grafo.escopo` e
`grafo.banca_alvo`. Nenhum desses campos se renomeia sem avisar a skill de aulas.
