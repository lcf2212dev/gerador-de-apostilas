...
```bash
S=.grok/skills/montar-sumario/scripts
python3 $S/sumario.py inspecionar grafos/<slug>
```
...
```bash
python3 $S/sumario.py priorizar grafos/<slug> \
...
declarado no sumário: `--minutos-aula` (padrão 30), `--beta` (peso da centralidade, padrão 0.35;
`0` desliga), `--sem-prerequisitos` (não puxa pré-requisito de fora do assunto).
...
sumário vira produção. Schema em `referencias/anatomia-do-sumario.md`.

### 6. Conferir — laço até aprovar

```bash
python3 $S/sumario.py conferir --plano sumarios/<slug>/plano.json \
...
```bash
python3 .grok/skills/apostila/scripts/contexto_aula.py sumarios/<slug>/plano.json --seq 1
```
...
| `scripts/testes/test_sumario.py` | 39 testes; rode depois de mexer em qualquer script |
