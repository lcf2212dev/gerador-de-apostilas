...
"""Testes de `sumario.py` e `ler_grafo.py`.
    ...
    python3 .grok/skills/montar-sumario/scripts/testes/test_sumario.py

Cobrem o que quebra em silêncio: ordem que viola pré-requisito, nota que não reage à
incidência, grafo com ciclo, grafo pobre em dados, e o `conferir` deixando passar uma
grade errada. Não testam gosto (nome de módulo, redação) — isso é do revisor humano.
"""
...
MOTOR = Path("/home/leandro/concurso") / ler_grafo.CAMINHO_MOTOR
