...
def raiz_projeto() -> Path:
    """A raiz do projeto, subindo de .grok/skills/mapear-conteudo/scripts/."""
...
def render(derivado: dict) -> str:
    ...
    p = [f"---",
         ...
         f"> e rodar `python3 .grok/skills/mapear-conteudo/scripts/grafo.py build {esc['slug']}`.",
         ""]
    ...
    p += ["## 8. Contrato de consumo", "",
          ...
          f"python3 $S consultar {esc['slug']} --classe A --json   # nós prioritários",
          f"python3 $S ordem {esc['slug']} --json                  # ordem de estudo (DAG)",
          f"python3 $S no {esc['slug']} <id>                       # ficha de um nó",
          f"python3 $S resumo {esc['slug']}                        # cobertura e confiança",
          "```",
          "",
