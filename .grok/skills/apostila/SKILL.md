...
name: apostila
description: >-
  Escreve uma aula completa em formato de apostila impressa — conteúdo didático de 20 a 45
  minutos de estudo, layout azul e verde sobre branco pensado para papel, seguido de 10 questões
...
| 3 a 6 seções de conteúdo, com caixas de sinalização | corpo |
...
```bash
A=.grok/skills/apostila/scripts
python3 $A/contexto_aula.py sumarios/<slug> --listar          # que aulas existem
python3 $A/contexto_aula.py sumarios/<slug> --aula 3          # briefing da aula 3
python3 $A/contexto_aula.py sumarios/<slug> --aula 3 --meta > meta.json
```
...
```bash
python3 $A/estimar_tempo.py --alvo 30 --orcamento     # quantas palavras e unidades cabem
python3 $A/estimar_tempo.py corpo.html --alvo 30      # onde você está + revisão didática
```
...
```bash
python3 $A/montar.py --dir aulas/<disciplina>/aula-NN-<slug>
bash    $A/gerar_pdf.sh aulas/<disciplina>/aula-NN-<slug>/<slug>.html
```

`montar.py` valida os contratos, escreve o HTML e imprime o relatório de tempo. Fora de
20–45 min ele sai com erro: ajuste o corpo e monte de novo. Durante a redação, `--parcial`
monta uma prévia sem as páginas finais.
...
```bash
python3 $A/fechar_aula.py --dir aulas/<disciplina>/aula-NN-<slug>
```
...
2. **O tempo nunca aparece na apostila.** Nem no cabeçalho, nem no mapa da aula, nem por seção.
   ...
   na cabeça.
3. **20 a 45 minutos de conteúdo**, sempre. Questões, gabarito e flashcards são extras e não
   entram na conta. Tema grande vira duas aulas; tema pequeno é aprofundado até o piso.
4. **Branco domina.** Cor só em filetes, ícones e fundos claros. Nada de bloco escuro: gasta
   toner e piora a leitura no papel.
5. **No máximo uma caixa a cada ~250 palavras.** Sinalização em excesso destrói a atenção que
   deveria criar. O validador de tempo avisa quando a densidade passa do ponto.
