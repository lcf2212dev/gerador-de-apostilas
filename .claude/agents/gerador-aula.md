...
(cwd do despacho) e use os caminhos literais abaixo. Não despache subagente
(a ferramenta Task não existe dentro de um subagente).
...
```bash
python3 .claude/skills/apostila/scripts/contexto_aula.py <SUMÁRIO> --aula <SEQ> --json
python3 .claude/skills/apostila/scripts/contexto_aula.py <SUMÁRIO> --listar
```
...
|---|---|---|---|
| 1 | `questoes-banca` | `questoes.json` | exatamente 10 itens; `python3 .claude/skills/questoes-banca/scripts/validar_questoes.py <DIRETÓRIO>/questoes.json --corpo <DIRETÓRIO>/corpo.html` |
...
```bash
python3 .claude/skills/apostila/scripts/montar.py --dir <DIRETÓRIO>
python3 .claude/skills/apostila/scripts/estimar_tempo.py <DIRETÓRIO>/corpo.html --alvo 30
```

Mire **o alvo do despacho ±5 min** (padrão 30 → 25–35). Trate os avisos de didática —
inclusive repetição e densidade: aula enxuta não é aula rasa, é aula sem eco. Quatro
rodadas fora da faixa: pare e devolva FALHA com a última estimativa.

### 4. PDF

```bash
bash .claude/skills/apostila/scripts/gerar_pdf.sh <DIRETÓRIO>/<slug>.html
```
