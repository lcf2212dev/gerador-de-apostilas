...
0. **QC, se houver conta.** Skill `qconcursos`:
   ```
   .venv/bin/python .claude/skills/qconcursos/scripts/qc.py aulas --disciplina "…" --assunto "…" --max 8
   ```
...
2. **Buscar candidatos:**
   ```
   python3 .claude/skills/videos-aula/scripts/yt.py buscar "sua consulta" --max 20
   ```
...
4. **Inspecionar os finalistas** (5 a 8 candidatos):
   ```
   python3 .claude/skills/videos-aula/scripts/yt.py inspecionar ID1 ID2 ID3 --json
   ```
