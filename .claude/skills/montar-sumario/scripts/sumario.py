...
acrescenta é o que falta para virar um plano de estudo:
  ...
  5. **regra de lastro** — incidência é pré-condição de aula cheia: item sem 4 questões
     na amostra e sem 2% do share vira aula curta ou opcional (`--sem-lastro` desliga).
...
PADROES = {
    ...
    "minutos_aula": 30,          # alvo por aula; a skill `apostila` trabalha em 20–45.
                                 # Alvo: o do briefing, ±5 min (padrão 30). Fora de
                                 # 20–45 o `montar.py` não monta.
    "minutos_min": 20,
    "minutos_max": 45,
    # Regra de lastro. O piso de 4 questões é o mesmo de
    # `mapear-conteudo/references/pesos.md`: "o piso de 4 questões evita que ruído de
    # amostra vire narrativa". Sem 4 questões e sem 2% do share, o item não sustenta
    # aula cheia — vira aula curta (até 30 min) ou opcional.
    "min_questoes_cheia": 4,
    "share_min_cheia": 0.02,
    "minutos_max_curta": 30,
...
def priorizar(g: Grafo, assunto: str, par: dict, importar_prereq: bool = True,
              sem_lastro: bool = False) -> dict:
    ...
    # --- regra de lastro: incidência é pré-condição de aula cheia ----------
    # O piso de 4 questões vem de `mapear-conteudo/references/pesos.md` ("o piso de 4
    # questões evita que ruído de amostra vire narrativa"). Item sem lastro vira aula
    # curta quando o edital exige ou algo com lastro depende dele, e opcional (0 aulas,
    # copiado para `omitidos` na grade) quando nem isso. Salvaguarda: sem amostra de
    # questões no recorte inteiro, a regra não julga ninguém.
    min_q = float(parametros["min_questoes_cheia"])
    share_min = float(parametros["share_min_cheia"])
    tem_incidencia = any((n.n_questoes or 0) > 0 or (n.share_pct or 0) > 0 for n in itens)
    sem_lastro_ids = {n.id for n in itens
                      if (n.n_questoes or 0) < min_q and share_de(n) < share_min}
    formato = {n.id: "cheia" for n in itens}
    motivo_omissao: dict[str, str] = {}
    lastro_aplicado = tem_incidencia and not sem_lastro
    if not tem_incidencia:
        sem_lastro_ids = set()          # sem incidência no recorte, ninguém é julgado
                                        # (cobre a base de valor "uniforme")
        if not sem_lastro:
            avisos.append("AVISO GRAVE: sem amostra de questões, a regra de lastro não "
                          "julga ninguém — todos os itens saíram como aula cheia")
    elif sem_lastro and sem_lastro_ids:
        avisos.append(f"--sem-lastro: {len(sem_lastro_ids)} item(ns) sem lastro "
                      "receberam aulas cheias mesmo assim — declare o motivo no sumário")
    if lastro_aplicado:
        cheias = {n.id for n in itens if n.id not in sem_lastro_ids}
        for n in itens:
            if n.id in cheias:
                continue
            if (n.edital or 0) > 0 or any(d in cheias for d in destrava.get(n.id, ())):
                formato[n.id] = "curta"
                minutos[n.id] = max(min(minutos[n.id], int(parametros["minutos_max_curta"])),
                                    int(parametros["minutos_min"]))
            else:
                formato[n.id] = "opcional"
                minutos[n.id] = 0
                q = int(n.n_questoes or 0)
                motivo_omissao[n.id] = (
                    f"opcional: {q} questão na amostra" if q == 1
                    else f"opcional: {q} questões na amostra"
                ) + ", sem exigência de edital, não destrava item cheio"
    ...
    for posicao, nid in enumerate(fila, 1):
        ...
        if nid in sem_lastro_ids:
            marcas.append("sem-lastro")
        if n.confianca is not None and n.confianca < parametros["limiar_confianca_baixa"]:
            marcas.append("confianca-baixa")
        fatias = [] if formato[nid] == "opcional" else fatiar_em_aulas(minutos[nid], parametros)
        saida_itens.append({
        ...
        if nid in motivo_omissao:
            saida_itens[-1]["motivo_omissao"] = motivo_omissao[nid]
    ...
    return {
        ...
        "parametros": {**{k: parametros[k] for k in PADROES}, "base_de_valor": modo_base,
                       ...
                       "lastro_aplicado": lastro_aplicado, "sem_lastro": sem_lastro},
        "totais": {
            ...
            "aulas_cheias": sum(i["aulas"] for i in saida_itens if i["formato"] == "cheia"),
            "aulas_curtas": sum(i["aulas"] for i in saida_itens if i["formato"] == "curta"),
            "itens_opcionais": sum(1 for i in saida_itens if i["formato"] == "opcional"),
...
def cmd_conferir(args) -> None:
    ...
    # a regra de lastro só é cobrada quando o plano diz que ela foi aplicada
    # (sob `--sem-lastro` — ou sem amostra — não há formato curta/opcional a exigir)
    lastro_aplicado = (plano.get("parametros") or {}).get("lastro_aplicado", False)
    for num, item in itens.items():
        if num not in por_item and num not in omitidos:
            defeitos.append(f"item {num} ({item['rotulo']}) não virou aula nem foi omitido "
                            "com justificativa")
            continue
        if lastro_aplicado and item.get("formato") == "curta" and num in por_item \
                and len(por_item[num]) != 1:
            defeitos.append(f"item {num} é aula curta no plano — exige exatamente 1 aula, "
                            f"a grade tem {len(por_item[num])}")
        if lastro_aplicado and item.get("formato") == "opcional" and num not in omitidos:
            defeitos.append(f"item {num} é opcional no plano: tem que estar em `omitidos` "
                            "da grade, com o motivo")
        if num in por_item:
...
def cmd_priorizar(args) -> None:
    ...
    if args.saida:
        ...
        if t.get("aulas_curtas") or t.get("itens_opcionais"):
            print(f"  lastro: {t['aulas_curtas']} aula(s) curta(s), "
                  f"{t['itens_opcionais']} item(ns) opcional(is)")
...
def main() -> None:
    ...
    s.add_argument("--sem-lastro", action="store_true",
                   help="desliga a regra de lastro: item sem incidência recebe aula "
                        "cheia mesmo assim (fica registrado no plano)")
