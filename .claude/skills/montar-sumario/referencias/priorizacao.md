...
O item vira `teto(minutos / 30)` aulas, cada uma dentro da faixa que a skill `apostila` aceita:
alvo o do briefing, ±5 min (padrão 30) — fora de 20–45 o `montar.py` dela não monta, e 45 é o
teto do fatiamento. Um tópico de 3 h vira 6 aulas de 30 min. Item sem lastro não fatia: vira
1 aula curta de até 30 min, ou nenhuma (ver §7).
...
### 7. Lastro — o que vira aula cheia, curta ou opcional

Cada item recebe um `formato`, calculado a partir de `n_questoes`, do `share` renormalizado no
recorte e do `edital`:

| formato | critério | efeito |
|---|---|---|
| `cheia` | `n_questoes` ≥ 4 **ou** `share` ≥ 2% | minutos normais, fatiamento normal |
| `curta` | sem lastro, mas `edital` > 0 **ou** é pré-requisito de item cheio | `min(minutos, 30)` — exatamente 1 aula |
| `opcional` | sem lastro, sem edital, não destrava item cheio | 0 aulas; vai para `omitidos` com o `motivo_omissao` do plano |

O piso de 4 questões não é invenção desta skill: é o mesmo de
`mapear-conteudo/references/pesos.md` — "o piso de 4 questões evita que ruído de amostra vire
narrativa".

Incidência é a moeda deste pipeline. Um tópico com zero questões na amostra pode ser lacuna da
amostra — mas alocar 7 aulas cheias nele é apostar 3,5 h do estudante num palpite. A aposta
certa é a inversa: uma aula curta quando o edital exige ou algo depende dele, a fila de
opcionais quando nem isso — e a promoção acontece quando `mapear-conteudo` trouxer questões,
não antes.

Duas válvulas de escape. Se o recorte inteiro não tem incidência (base de valor uniforme), a
regra desliga sozinha, com aviso grave: sem amostra de questões, ela não julga ninguém. E
`--sem-lastro` desliga por decisão do usuário — o item sem lastro sobe a aula cheia com a marca
`sem-lastro`, o plano registra a flag em `parametros`, e o sumário precisa declarar o motivo.
...
| `--minutos-aula` | 30 | 20 para blocos curtos, 45 para blocos longos — a produção mira o alvo ±5 min, e fora de 20–45 o `montar.py` da `apostila` não monta (45 segue sendo o teto do fatiamento) |
...
| `--sem-lastro` | desligado | quando a amostra é sabidamente lacunar e o usuário aceita aula cheia sem lastro — declare o motivo no sumário |
...
| muitos itens curtos/opcionais | a amostra é pequena; o defeito está no grafo, não na regra — volte a `mapear-conteudo` |
