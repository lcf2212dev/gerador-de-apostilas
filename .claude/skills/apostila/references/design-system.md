...
Todas seguem `<aside class="box TIPO"><h4>Rótulo</h4>…</aside>`. **Uma a cada ~400 palavras,
no máximo dez por aula** (alvo de uma aula de 30 min: 6 a 8). O validador acusa densidade acima
de 1 caixa/350 palavras. Impresso, isso dá em média **no máximo um bloco sinalizado por
página** — sinalização em dobro é atenção pela metade.
...
Cada tipo tem teto: `chave`, no máximo **1 por seção** — se tudo é chave, nada é;
`mnemonico`, no máximo **2 por aula**; `lei`, só quando a letra do dispositivo é cobrada
literalmente — se a paráfrase serve, ela vai no texto corrido.

## Blocos didáticos

Diferentes das caixas: fazem parte do fluxo do texto, vêm sem fundo colorido e **não entram no
limite de densidade**. `.exemplo` e `.passo-a-passo` não têm limite — são eles que transformam
enunciado em ensino. **`.em-outras-palavras` tem: no máximo 3 por aula**, e só depois de lei
seca ou definição técnica — não é resumo de seção. O rótulo é gerado pelo CSS; não escreva
título dentro deles.
...
<div class="checkpoint">
  <h4>Checkpoint da aula</h4>
  <ul>
    <li>Sei recitar os cinco fundamentos na ordem correta.</li>
    <li>Distingo fundamento de objetivo sem hesitar.</li>
    <li>Sei apontar onde o parágrafo único ancora a soberania popular.</li>
  </ul>
</div>
```

**Um checkpoint por aula**, imediatamente antes do quadro-síntese, com **3 a 5** afirmações
auto-verificáveis — uma por seção (o estudante marca a caixinha). Escreva no indicativo —
"sei…", "distingo…" —, nunca como pergunta.
...
Uma linha por seção da aula. `montar.py` acrescenta sozinho, depois disso, as fontes
consultadas e o rodapé — não escreva esses blocos. A revisão espaçada fica nos flashcards
(Anki), não numa faixa impressa.
...
- Parágrafos de 3 a 6 linhas. Bloco maior que isso, na coluna de leitura (156 mm), vira parede.
- Listas com no máximo 7 itens; acima disso, tabela ou divisão em duas.

## Margem de anotação

Desligada por padrão. Margens da folha: 16 mm em toda a volta; a coluna de leitura (156 mm)
fica centrada na folha. Quando ligada (`meta.json`), a coluna encosta à esquerda e sobra uma
faixa de ~22 mm à direita para anotações.
