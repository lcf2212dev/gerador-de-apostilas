...
Valores vêm do `progresso.py --pendentes --json`. Despache com a ferramenta **Task**,
`subagent_type: gerador-aula` ou `revisor-aula` (agentes de `.claude/agents/`).
`cwd` na raiz do repo. Os agentes herdam o modelo da sessão (`model: inherit`).
...
DURAÇÃO ALVO: <minutos da grade> min (±5; trava 20–45)
...
- O revisor **não** é filho do gerador (subagente não despacha subagente no Claude Code).
