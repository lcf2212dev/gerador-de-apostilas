...
description: >-
  ...
  estudo e produz TODAS as aulas em PDF (~30 min cada), encadeando as skills
...
python3 .claude/skills/curso-completo/scripts/progresso.py sumarios/<slug>   # se houver sumário
```
...
```bash
python3 .claude/skills/montar-sumario/scripts/sumario.py conferir \
...
```bash
python3 .claude/skills/curso-completo/scripts/progresso.py sumarios/<slug> --pendentes --limite 16 --json
```

1. O comando devolve os próximos 16 seqs (ou os que restarem) com o **diretório
   exato** de cada um.
2. Lote A: **até 16× a ferramenta Task com `subagent_type: gerador-aula`** na mesma
   mensagem — diretórios disjuntos. O Claude Code roda em paralelo até o limite de
   concorrência e enfileira o restante do lote sozinho. Template em
   `referencias/orquestracao.md`.
3. Lote B, **depois que o A voltar**: **até 16× `revisor-aula`** nos mesmos diretórios.
   Subagente não despacha subagente — o review não sai de dentro do gerador.
...
```bash
python3 .claude/skills/curso-completo/scripts/progresso.py sumarios/<slug>
```
...
```bash
python3 .claude/skills/flashcards-anki/scripts/anki_import.py aulas/<disciplina>/aula-NN-<slug>
```
...
| `.claude/agents/gerador-aula.md` | escreve cada aula (`model: inherit`) |
| `.claude/agents/revisor-aula.md` | double-check (`model: inherit`) |
| `.claude/skills/revisar-aula/` | rubrica e contrato do `revisao.json` |
