...
```bash
python3 .claude/skills/mapear-conteudo/scripts/grafo.py init <slug> --escopo "Direito Constitucional" --tipo disciplina \
...
(só da sessão principal — subagente não despacha subagente), com a taxonomia fechada no prompt (modelo em
...
```bash
python3 .claude/skills/mapear-conteudo/scripts/grafo.py validar <slug> && python3 .claude/skills/mapear-conteudo/scripts/grafo.py build <slug>
```
