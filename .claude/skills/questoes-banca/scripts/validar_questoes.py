...
def normalizar_com_acentos(texto: str) -> str:
    # Acento é distintivo em alternativa ("à — a — à" ≠ "a — à — a", formato
    # clássico de item de crase); aqui só caem tags, pontuação e caixa.
    sem_tags = re.sub(r"<[^>]+>", "", texto or "")
    minusculo = unicodedata.normalize("NFC", sem_tags.lower())
    return " ".join(re.sub(r"[^\w\s]", " ", minusculo).split())
...
def validar(dados: dict, total_secoes: int | None) -> tuple[list, list]:
    ...
    for i, q in enumerate(questoes, 1):
        ...
        textos_acentuados = {l: normalizar_com_acentos(t) for l, t in alts.items()}
        vistos = {}
        for l, t in textos_acentuados.items():
